...
If the build step fails immediately with errors such as `cannot connect to the Docker daemon` or `permission denied while trying to connect to the Docker socket`, ensure the Docker engine is running and that your user belongs to the `docker` group. GPU-enabled builds also require the [NVIDIA Container Toolkit](https://docs.nvidia.com/datacenter/cloud-native/container-toolkit/install-guide.html) to be installed on the host system.
